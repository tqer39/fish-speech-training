...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model-name", "-M", type=str, help="モデル名を指定します。"
    )
    parser.add_argument(
        "--force", "-F",
        action="store_true",
    ...
    parser.add_argument(
        "--directory", "-D", required=True, help="[REQUIRED] YYMMDD_HHMMSS フォーマットのディレクトリ名を指定します。"
    )
    parser.add_argument(
        "--override-path",
...
def main(args: Optional[Namespace] = None) -> None:
    ...
    target_dir = (
        args.override_path or f"./data/{model_name}/raw/{args.directory}/before_text_reformatting"
    )
